...
The production app uses a hosted PostgreSQL database. Set `POSTGRES_URL_NON_POOLING`, `ADMIN_PIN`, and `SESSION_SECRET` before deploying. When Neon is connected through Vercel, it supplies `POSTGRES_URL_NON_POOLING` automatically. Run locally with `npm run dev`, then open `http://localhost:3000`.
